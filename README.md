...
## 上古汉语音节表维护与查询工具
[上古汉语音节表](https://zhuanlan.zhihu.com/p/12987993957)是新最小上古汉语(NOCM)成果的记录与直接反映。包含音节表、字典表、小韵表3张表，音节表可用于观察声韵搭配，字典表用于记录字词释义、语文学考释、[线上查询](https://qwert-ly.github.io/xtext/)，小韵表用于[汉字音典APP](https://github.com/osfans/MCPDict)查询
...
- `clust_lda.py`：提取文本词频矩阵，进行隐含狄利克雷分布（Latent Dirichlet Allocation）软聚类
- `clust_nmf.py`：提取文本TF-IDF矩阵（高频词过滤），进行非负矩阵分解（NMF）软聚类
