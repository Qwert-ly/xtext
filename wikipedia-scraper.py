    ...
    def getWikiItem(cls, url):
        def find_content(soup):
            ...
            for i in content.find_all('h2'):
                if i.has_attr('id') and i['id'].startswith('参') or i['id'].startswith('阅'):
                    i.extract()
            for i in content.find_all('div', class_='reflist'):
                i.extract()
            for i in content.find_all('div', class_='refbegin'):
                i.extract()
            for i in content.find_all('table', class_='navbox'):
                i.extract()
            return content

        def find_cls(soup):
            footer = soup.find('div', class_='mw-normal-catlinks')
            cls_list = []
            for a in footer.find_all('a'):
                if a.has_attr('href') and a['href'].startswith('/wiki'):
                    cls_list.append(a.contents[0])
            return cls_list
...
while True:
    toko = input()
    url = f'https://zh.wikipedia.org/zh-hant/{toko}'
    M = WikipediaItem.getWikiItem(url)
    M.save_to_txt(M.text, f'{sys.path[0]}\\ctext - 白话 - slice/wiki-{toko}.txt')
